...
describe('validateRoundTerms', () => {
  /**
   * The two guards that came with sequence mode. Both exist because a round
   * that opens by itself is a treasury cap opening by itself.
   */
  it('refuses a round that opens by itself but cannot price', () => {
    const v = validateRoundTerms([{ round: 1, opensAt: null, discountPercent: null, opensMode: 'sequence' }]);
    expect(v.ok).toBe(false);
    expect(v.error).toMatch(/discount/i);
  });

  it('refuses a later round with its own date while an earlier one waits its turn', () => {
    // Round 3 on a date could otherwise open while round 2 is still waiting on
    // round 1 — the published order run backwards, and the date check below
    // cannot see it because that one only runs when every round has a date.
    const v = validateRoundTerms([
      { round: 1, opensAt: '2026-09-09T05:53:00Z', discountPercent: 22, opensMode: 'date' },
      { round: 2, opensAt: null, discountPercent: 25, opensMode: 'sequence' },
      { round: 3, opensAt: '2026-10-01T00:00:00Z', discountPercent: 28, opensMode: 'date' },
    ]);
    expect(v.ok).toBe(false);
    expect(v.error).toMatch(/round 3/);
  });

  it('but a sequence suffix is fine, and so is a date kept as a backstop', () => {
    const v = validateRoundTerms([
      { round: 1, opensAt: '2026-09-09T05:53:00Z', discountPercent: 22, opensMode: 'date' },
      { round: 2, opensAt: '2026-10-26T00:54:00Z', discountPercent: 25, opensMode: 'sequence' },
      { round: 3, opensAt: null, discountPercent: 28, opensMode: 'sequence' },
    ]);
    expect(v.ok).toBe(true);
    expect(v.rows.map(r => r.opensMode)).toEqual(['date', 'sequence', 'sequence']);
  });

  it('an unknown mode is refused rather than quietly ignored', () => {
    expect(validateRoundTerms([{ round: 1, opensAt: null, discountPercent: 22, opensMode: 'whenever' as any }]).ok)
      .toBe(false);
  });
...
/**
 * THE ROUND THAT OPENS WHEN THE ONE BEFORE IT RUNS OUT.
 *
 ...
 *
 * The rules live in lib/roundSequence.ts and are tested there against the
 * predicate itself. These are about the gate: that the decision a seller
 * actually meets agrees with them, and — the part that matters most — that a
 * round which was never given any way to open is still shut, exactly as it was
 * before any of this existed.
 */
describe('a round whose turn has come', () => {
  /** Terms that carry a mode instead of a date. */
  const seq = (round: number, discount = 22, openedAt: number | null = null): RoundTerms =>
    ({ round, opensAt: null, discountPercent: discount, opensMode: 'sequence', openedAt });

  it('round 1 in sequence mode opens with the window, with no date anywhere', () => {
    const v = decided(evaluateRoundMandate(input({ terms: [seq(1, 22, NOW - 10)] })));
    expect(v.outcome).toBe('accept');
    expect(v.discountPercent).toBe(22);
  });

  it('round 2 opens once its turn has been recorded', () => {
    const v = decided(evaluateRoundMandate(input({
      candidates: [mandate({ lanaReceivedLanoshis: 0 }), r2()],
      terms: [seq(1), seq(2, 25, NOW - 10)],
    })));
    expect(v.outcome).toBe('accept');
    expect(v.round).toBe(2);
    expect(v.discountPercent).toBe(25);
  ...
  /**
   * Before the turn is recorded the round is shut — and the seller is told the
   * truth about WHY, which is not "we have not published terms" and not a date
   * they can count down to.
   */
  it('and before that says it follows the round in front of it, with no date', () => {
    const v = decided(evaluateRoundMandate(input({
      candidates: [mandate({ lanaReceivedLanoshis: 0 }), r2()],
      terms: [seq(1), seq(2, 25)],
    })));
    expect(v.outcome).toBe('decline');
    expect(v.code).toBe('AWAITING_TURN');
    expect(v.round).toBe(2);
    expect(v.reason).toMatch(/round 1/);
    expect(v.reason).not.toMatch(/\d{4}-\d{2}-\d{2}/);
  });

  /**
   * NOTHING CHANGES FOR A ROUND THAT DID NOT ASK. The whole risk of this
   * change is the round nobody touched, so: a date still opens on its date, a
   * bare row with neither is still TERMS_MISSING, and a discount alone still
   * buys nothing.
   */
  it('leaves a dated round exactly as it was', () => {
    expect(decided(evaluateRoundMandate(input({ terms: [open(1, 22, NOW - 1)] }))).outcome).toBe('accept');
    expect(decided(evaluateRoundMandate(input({ terms: [open(1, 22, NOW + 1)] }))).code).toBe('MANDATE_NOT_OPEN');
    expect(decided(evaluateRoundMandate(input({ terms: [{ round: 1, opensAt: null, discountPercent: 22 }] }))).code)
      .toBe('TERMS_MISSING');
  });

  /**
   * The date and the turn never push each other back. Split 8 round 2 carries
   * 26 October: a recorded turn may open it sooner, and nothing can make a
   * financer wait past the date they were already shown.
   */
  it('a date and a turn are not rivals — the earlier one wins, both ways', () => {
    const dated = { round: 1, opensAt: NOW + 86_400, discountPercent: 22, opensMode: 'sequence' as const };
    expect(decided(evaluateRoundMandate(input({ terms: [dated] }))).code).toBe('MANDATE_NOT_OPEN');
    expect(decided(evaluateRoundMandate(input({ terms: [{ ...dated, openedAt: NOW - 1 }] }))).outcome).toBe('accept');
    // …and a turn recorded for later cannot delay a date that has arrived.
    expect(decided(evaluateRoundMandate(input({
      terms: [{ round: 1, opensAt: NOW - 1, discountPercent: 22, opensMode: 'date' as const, openedAt: NOW + 86_400 }],
    }))).outcome).toBe('accept');
  });

  it('a sequence round with no discount still cannot price, turn or no turn', () => {
    const priceless: RoundTerms = { round: 1, opensAt: null, discountPercent: null, opensMode: 'sequence', openedAt: NOW - 10 };
    expect(decided(evaluateRoundMandate(input({ terms: [priceless] }))).code).toBe('TERMS_MISSING');
  });

  it('round 1 is still tried before round 2, even with both open', () => {
    const v = decided(evaluateRoundMandate(input({
      candidates: [mandate(), r2()],
      terms: [seq(1, 22, NOW - 10), seq(2, 25, NOW - 10)],
    })));
    expect(v.round).toBe(1);
  });
});
