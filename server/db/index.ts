...
db.exec(`
  ...
  -- LanaCrowd donations (KIND 60200). RETIRED 4 Sep 2026 with the old
  -- settlement order: nothing reads or writes these two tables any more.
  -- Kept (no DROP) so existing rows stay auditable.
...
export function getAllSalesWithPayouts(): any[] {
  ...
  // Each sale carries the financing round of the offer it came from and the
  // split its mandate was published for (offer_ref → acquisition_offers →
  // acquisition_mandates). Both null on a legacy sale or one outside a round.
  // This is the ONE join the public board and the admin screen order by.
  const getSales = db.prepare(`
    SELECT bt.*, ao.round AS offer_round, ao.accepted_at AS offer_accepted_at, am.split AS mandate_split
    FROM buyback_transactions bt
    LEFT JOIN acquisition_offers ao ON ao.offer_ref = bt.offer_ref
    LEFT JOIN acquisition_mandates am ON am.d_tag = ao.mandate_ref
    WHERE bt.user_hex_id = ? AND bt.status IN ('broadcast', 'completed', 'paid', 'pending_verification')
    ORDER BY bt.created_at DESC
  `);
    ...
    return {
      ...
      sales: sales.map(sale => {
        ...
        return {
          ...
          offerRef: sale.offer_ref ?? null,
          settlementDueAt: sale.settlement_due_at ?? null,
          round: sale.offer_round ?? null,
          mandateSplit: sale.mandate_split ?? null,
          acceptedAt: sale.offer_accepted_at ?? null,
          totalPaid: Math.round(totalPaid * 100) / 100,
          remaining: remaining <= 0 ? 0 : remaining,
          payouts: payouts.map(p => ({
            id: p.id,
